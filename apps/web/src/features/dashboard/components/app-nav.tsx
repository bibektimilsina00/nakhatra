...
export function AppNav({
  ...
  const panel =
    "absolute right-0 z-50 mt-2 rounded-[8px] border border-white/12 bg-[#0B0E18] p-1.5 shadow-2xl shadow-black/50";
  const trigger =
    "flex items-center gap-2 rounded-[8px] border border-white/10 text-muted transition-colors hover:border-white/25 hover:text-paper";

  return (
    <header className="sticky top-0 z-40 border-b border-white/[0.06] bg-ink2">
      <div ref={bar} className="flex items-center gap-4 px-5 py-3 sm:px-8">
        {onOpenMenu && (
          <button
            ...
            className="rounded-[8px] p-1.5 text-muted transition-colors hover:text-paper lg:hidden"
          >
        ...
        <label className="relative hidden max-w-[380px] flex-1 items-center md:flex">
          ...
          <input
            ...
            className="w-full rounded-[8px] border border-white/[0.09] bg-card/50 py-2 pl-9 pr-10 text-[13.5px] text-paper placeholder-faint focus:border-gold/45 focus:outline-none"
          />
          {query ? (
            <button
              type="button"
              ...
              className="absolute right-2 rounded px-1.5 text-[11px] text-faint hover:text-paper"
            >
              ✕
            </button>
          ) : (
            <span className="absolute right-2.5 text-[10px] text-faint">⌘K</span>
          )}
        </label>
        )}
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <div ref={bell} className="relative">
            ...
            >
              ...
              {unread > 0 && (
                <span className="absolute -right-1 -top-1 grid size-[17px] place-items-center rounded-full bg-gold text-[9.5px] font-bold text-ink">
                  {unread}
                </span>
              )}
            </button>
            ...
            {open === "bell" && (
              <div className={`${panel} w-[320px] !p-0`}>
                <div className="flex items-center justify-between gap-3 border-b border-white/[0.08] px-3.5 py-2.5">
                  <span className="text-[13px] font-semibold text-paper">{t.dashNotifications}</span>
                  ...
                  {unread > 0 && (
                    <button
                      type="button"
                      onClick={markAllRead}
                      className="inline-flex items-center gap-1.5 text-[11px] text-muted transition-colors hover:text-gold"
                    >
                    ...
                    {items.map((item) => (
                      <li key={item.id}>
                        <div className="flex gap-3 border-b border-white/[0.05] px-3.5 py-3 last:border-0">
                          <span
                            className={`mt-1.5 size-1.5 shrink-0 rounded-full ${item.unread ? "bg-gold" : "bg-transparent"}`}
                          />
                          <span className="min-w-0">
                            <span className="block truncate text-[13px] leading-[1.5] text-paper">
                              {item.title}
                            </span>
                ...
                <Link
                  href="/notifications"
                  onClick={close}
                  className="flex items-center justify-center gap-1.5 border-t border-white/[0.08] px-3.5 py-2.5 text-[12px] text-muted transition-colors hover:text-gold"
                >
                  {t.notifSeeAll}
                  {total > items.length && (
          ...
          <div ref={account} className="relative">
            <button
              ...
              className="grid size-9 place-items-center rounded-full bg-gold text-[13.5px] font-bold text-ink"
            >
            ...
            {open === "account" && (
              <div className={`${panel} w-[236px]`}>
                <div className="flex items-center gap-2.5 px-2 py-2">
                  <span className="grid size-9 shrink-0 place-items-center rounded-full bg-gold text-[13.5px] font-bold text-ink">
                    {user.full_name.trim().charAt(0).toUpperCase()}
                  </span>
                  <span className="min-w-0">
                ...
                <button
                  type="button"
                  onClick={logout}
                  className="flex w-full items-center gap-2.5 rounded-[6px] px-2.5 py-2 text-left text-[13px] text-muted transition-colors hover:bg-white/[0.05] hover:text-paper"
                >

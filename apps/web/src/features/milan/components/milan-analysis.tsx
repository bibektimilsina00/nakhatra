...
export function MilanAnalysisPanel({
  ...
  if (isError || !analysis) {
    return (
      <section className="rounded-[10px] border border-white/[0.09] bg-card p-6 text-center">
        <p className="text-[13px] text-faint">{t.milanAnalysisFailed}</p>
        <button
          type="button"
          onClick={onRetry}
          className="mt-4 inline-flex items-center gap-2 rounded-[8px] border border-white/12 px-4 py-2 text-[12.5px] text-muted transition-colors hover:border-white/25 hover:text-paper"
        >
  ...
  return (
    <div className="space-y-4">
      <section className="rounded-[10px] border border-white/[0.09] bg-card p-6">
        <h2 className={`text-[10.5px] text-gold ${label}`}>{t.milanAnalysisTitle}</h2>
        <p className="mt-3 text-[17px] font-semibold leading-snug text-paper">
          {analysis.verdict}
        </p>
        <p className="mt-3 text-[13.5px] leading-[1.75] text-muted">{analysis.outlook}</p>
        <p className="mt-4 border-t border-white/[0.07] pt-3.5 text-[11.5px] leading-[1.6] text-faint">
          {t.milanAnalysisNote}
        </p>
      </section>
      ...
      {doshas.length > 0 && (
        <section className="rounded-[10px] border border-white/[0.09] bg-card p-5">
          <h3 className="mb-4 flex items-center gap-2 text-[13px] font-semibold text-paper">
            <ShieldAlert className="size-4 text-faint" />
            {t.milanDoshas}
          </h3>
      ...
      {remedies.length > 0 && (
        <section className="rounded-[10px] border border-white/[0.09] bg-card p-5">
          <h3 className="mb-4 flex items-center gap-2 text-[13px] font-semibold text-paper">
            <HandHeart className="size-4 text-faint" />
            {t.milanRemedies}
          </h3>
          ...
          <ul className="grid gap-3 sm:grid-cols-2">
            {remedies.map((remedy) => (
              <li key={remedy.title} className="rounded-[8px] border border-white/[0.07] bg-ink p-4">
                <div className="flex items-baseline justify-between gap-3">
                  <p className="text-[13px] font-medium text-paper">{remedy.title}</p>
                  <span className="shrink-0 text-[10.5px] text-gold">{remedy.timing}</span>
                </div>
                <p className="mt-1.5 text-[12.5px] leading-[1.7] text-faint">{remedy.detail}</p>
              </li>
            ))}
          </ul>
        </section>
...
function PointList({
  ...
  const colour = tone === "good" ? "text-emerald-400/80" : "text-gold";

  return (
    <section className="h-full rounded-[10px] border border-white/[0.09] bg-card p-5">
      <h3 className="mb-4 flex items-center gap-2 text-[13px] font-semibold text-paper">
      ...
      <ul className="space-y-4">
        {points.map((point) => (
          <li key={point.title}>
            <p className="text-[13px] font-medium text-paper">{point.title}</p>
            <p className="mt-1.5 text-[12.5px] leading-[1.7] text-muted">{point.detail}</p>
...
function DoshaRow({ dosha, affectsLabel }: { dosha: MilanDosha; affectsLabel: string }) {
  ...
  const severity = {
    none: { text: t.milanSeverityNone, cls: "border-white/[0.10] bg-ink text-faint" },
    mild: { text: t.milanSeverityMild, cls: "border-gold/25 bg-[#1A150B] text-gold2" },
    moderate: { text: t.milanSeverityModerate, cls: "border-gold/35 bg-[#1A150B] text-gold" },
    serious: { text: t.milanSeveritySerious, cls: "border-rose-400/30 bg-[#1A1013] text-rose-300/90" },
  }[dosha.severity];

  return (
    <li className="rounded-[8px] border border-white/[0.07] bg-ink p-4">
      <div className="flex items-baseline justify-between gap-3">
        <p className="text-[13px] font-medium text-paper">{dosha.name}</p>
        <span
      ...
      <p className="mt-2 text-[12.5px] leading-[1.7] text-muted">{dosha.detail}</p>
      {dosha.affects && (
        <p className="mt-2 text-[11px] text-faint">
...
function Analysing() {
  ...
  return (
    <section className="rounded-[10px] border border-white/[0.09] bg-card p-6">
      <h2 className={`text-[10.5px] text-gold ${label}`}>{t.milanAnalysisTitle}</h2>
      <p className="mt-3 flex items-center gap-2 text-[14px] font-medium text-paper">
        <Sparkle className="size-4 animate-pulse text-gold" />
        {t.milanAnalysing}
      </p>
      <p className="mt-2 max-w-md text-[12.5px] leading-[1.7] text-faint">
        {t.milanAnalysingNote}
      </p>
      <div className="mt-5 space-y-2.5">
        {[100, 92, 78].map((w) => (
          <div key={w} className="h-2.5 animate-pulse rounded-full bg-ink" style={{ width: `${w}%` }} />
        ))}
      </div>
    </section>
  );

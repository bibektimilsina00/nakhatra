  ...
  return (
    <AppShell>
      <main className="mx-auto w-full max-w-[560px] px-5 pb-24 pt-24 sm:px-8">
        <div className="rounded-[14px] border border-white/[0.09] bg-card p-8 text-center">
          <span className="mx-auto grid size-12 place-items-center rounded-full border border-gold/25 text-gold">
            <Sparkles className="size-5" />
          </span>
          <span className={`mt-5 block text-[11px] text-gold ${eyebrow}`}>{t.practComingSoon}</span>
          <h1 className="mt-3 text-[22px] font-bold leading-tight text-paper">{t.dashJyotish}</h1>
          <p className="mx-auto mt-3 max-w-sm text-[13.5px] leading-[1.8] text-muted">
            {t.soonNote}
          </p>
          <Link
            href="/dashboard"
            className="mt-6 inline-block rounded-[9px] bg-gold px-5 py-2.5 text-[13px] font-semibold text-ink transition-colors hover:bg-gold2"
          >
            {t.dashNavHome}
          </Link>
        </div>

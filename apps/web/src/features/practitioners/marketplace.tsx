  ...
  return (
    <AppShell>
      <main className="mx-auto w-full max-w-[560px] px-5 pb-24 pt-24 sm:px-8">
        <div className="rounded-lg border border-line-strong bg-surface p-8 text-center">
          <span className="mx-auto grid size-12 place-items-center rounded-full border border-line-strong text-accent-strong">
            <Sparkles className="size-5" />
          </span>
          <span className={`mt-5 block text-2xs text-accent-strong ${eyebrow}`}>{t.practComingSoon}</span>
          <h1 className="mt-3 text-xl font-bold leading-tight text-ink">{t.dashJyotish}</h1>
          <p className="mx-auto mt-3 max-w-sm text-sm leading-[1.8] text-muted">
            {t.soonNote}
          </p>
          <Link
            href="/dashboard"
            className="mt-6 inline-block rounded-md bg-accent-strong px-5 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-accent-ink"
          >
            {t.dashNavHome}
          </Link>
        </div>

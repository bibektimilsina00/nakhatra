...
export function PractitionerCardView({
  ...
  return (
    <button
      ...
      className="flex w-full flex-col rounded-[12px] border border-white/[0.09] bg-card p-5 text-left transition-colors hover:border-gold/35"
    >
      <div className="flex items-start gap-3">
        <span className="grid size-11 shrink-0 place-items-center rounded-[10px] bg-gold/[0.12] text-[14px] font-bold text-gold">
          {initials}
        </span>
        <span className="min-w-0 flex-1">
          <span className="flex items-center gap-1.5">
            ...
            {practitioner.verified && (
              <BadgeCheck className="size-4 shrink-0 text-gold" aria-label={t.dashVerified} />
            )}
          </span>
      ...
      {(practitioner.specialities.length > 0 || practitioner.languages.length > 0) && (
        <span className="mt-4 flex flex-wrap gap-1.5">
          ...
          {practitioner.languages.slice(0, 3).map((tag) => (
            <span
              key={tag}
              className="rounded-[6px] border border-white/[0.06] bg-ink px-2 py-0.5 text-[10.5px] uppercase text-faint"
            >
              {tag}
            </span>
          ))}
